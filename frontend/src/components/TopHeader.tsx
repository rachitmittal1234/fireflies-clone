...
export default function TopHeader() {
  ...
  return (
    <header className="h-14 border-b border-[var(--ff-border)] bg-white flex items-center justify-end gap-3 px-6 sticky top-0 z-20">
      {/* Notifications */}
      <div className="relative" ref={notifRef}>
        <button
          onClick={() => setNotifOpen((o) => !o)}
          className="w-9 h-9 rounded-full hover:bg-gray-50 flex items-center justify-center text-[var(--ff-text-muted)] hover:text-[var(--ff-text)] transition relative"
          title="Notifications"
        >
      ...
      <div className="relative" ref={profileRef}>
        ...
        {profileOpen && (
          <div className="absolute right-0 mt-2 w-56 ff-card p-2 shadow-xl z-30">
            ...
            <button className="w-full text-left px-3 py-2 text-sm rounded-lg hover:bg-gray-50 text-red-500 transition">
              Log Out
            </button>
          </div>
        )}
